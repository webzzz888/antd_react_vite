...
export const getStorage = (key: string) => {
  ...
  const storage = config.isEncrypt
    ? JSON.parse(decrypt(window[config.type].getItem(prefixKey)))
    : JSON.parse(window[config.type].getItem(prefixKey));
...
export const getStorageAll = () => {
  ...
  for (let i = 0; i < len; i++) {
    ...
    // 获取key对应的值
    const storage = config.isEncrypt
      ? JSON.parse(decrypt(window[config.type].getItem(typeof key === 'string' ? key : '')))
      : JSON.parse(window[config.type].getItem(typeof key === 'string' ? key : ''));
